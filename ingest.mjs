// ingest.mjs — Ingest + link + novelty rank + basic scoring (multi-assignment)
...
// Multi-assignment support (comma/space separated). Legacy ASSIGNMENT_ID still works.
const IDS_RAW =
  process.env.ASSIGNMENT_IDS ||
  process.env.ASSIGNMENT_ID ||
  "";

const ASSIGNMENT_IDS = IDS_RAW.split(/[,\s]+/)
  .map(s => s.trim())
  .filter(Boolean);
...
// Novelty bucketing (matches our SQL rpc expectations)
function noveltyKey(rubric, obs) {
  ...
  const bucket = new Date(
    Math.floor(t.getTime() / (hrs * 3600e3)) * (hrs * 3600e3)
  ).toISOString();
...
async function rankNoveltyForAssignment(assignmentId) {
  await http("POST", `${PGR}/rpc/rank_novelty_for_assignment`,
    { p_assignment: assignmentId }, HDR);
}
...
async function readAssignment(assignmentId) {
  ...
  const { json } = await http("GET", `${PGR}/assignments?${qp.toString()}`, null, HDR);
  if (!json?.length) {
    // small recent snapshot for debugging
    ...
    const snapshot = Array.isArray(list)
      ...
      : "none";
    throw new Error(
      `Assignment not found for '${assignmentId}'. Recent: ${snapshot}`
    );
  }
  const a = json[0];
...
function localDayCR(iso) {
  ...
  if (isNaN(t)) return null;
  // cheap CR timezone day-bucketing (UTC-6, allow wiggle)
...
async function writeScores(assignmentId, byStudent, meta) {
  const runs = [{
    ...
    observations_fetched: meta?.totalFetched ?? 0, // <-- real count logged
    notes: `GH Action basic scoring; fetched=${meta?.totalFetched ?? 0}`
  }];

  const { json } = await http("POST", `${PGR}/score_runs?on_conflict=id`, runs, HDR);
...
async function processAssignment(ASSIGN) {
  ...
  if (!userLogins.length) {
    console.warn(`No iNat usernames in student_identities. Skipping '${ASSIGN}'.`);
    return;
  }
  ...
  for await (const page of inatObsPager(params)) {
    ...
    await upsert("assignment_observations", mapLinks(ASSIGN, idsMap, rubric, mapped),
      "(assignment_id,inat_obs_id)");
    await sleep(throttleMs);
  }

  // novelty (ranks per assignment)
