...
// Helpful debug (booleans only so secrets never print)
...
const ASSIGN_IDS = IDS_RAW.split(",").map(s => s.trim()).filter(Boolean);
...
// Upsert with proper on_conflict usage (comma list, no parens)
async function upsert(table, rows, onConflictCols) {
  if (!rows?.length) return;
  const CH = 200;
  const h = { ...HDR, Prefer: "resolution=merge-duplicates" };
  const oc = encodeURIComponent(onConflictCols);
  for (let i = 0; i < rows.length; i += CH) {
    await http("POST", `${PGR}/${table}?on_conflict=${oc}`, rows.slice(i, i + CH), h);
    await sleep(50);
  }
}
...
async function readAssignment(ASSIGN) {
  ...
  const { json } = await http("GET", `${PGR}/assignments?select=${sel}&id=eq.${ASSIGN}`, null, HDR);
  if (!json?.length) {
    ...
    throw new Error(`Assignment not found for '${ASSIGN}'. Recent: ${snapshot.join(", ") || "none"}`);
  }
...
async function writeScores(ASSIGN, byStudent, maxUpdated) {
  ...
  const { json: runIns } = await http("POST", `${PGR}/score_runs`, runRow, { ...HDR, Prefer: "return=representation" });
  const runId = runIns?.[0]?.id;
  if (!runId) throw new Error("score_run insert failed");

  const entries = [];
  for (const [sid, m] of byStudent) {
    entries.push({ score_run_id: runId, assignment_id: ASSIGN, student_id: sid, total_points: m.total_points, breakdown_json: m });
  }
  if (entries.length) await http("POST", `${PGR}/score_entries`, entries, HDR);
...
(async () => {
  try {
    for (const id of ASSIGN_IDS) {
      ...
      catch (e) { console.error(`✗ ${id}: ${String(e.message || e)}`); }
    }
    console.log("OK");
  } catch (e) {
